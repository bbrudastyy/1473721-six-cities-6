...
const getCitiesContainer = (offersList, offersCount, city) => {
  if (offersList.length !== 0) {
    return <div className="cities__places-container container">
      <section className="cities__places places">
        ...
        <CardContainer hotels={offersList} containerType={TypeCard.MAIN} />
      </section>
      <div className="cities__right-section">
        <Map hotels={offersList} type={TypeMap.CITIES} />
      </div>
    </div>;
  } else {
...
const MainScreen = (props) => {
  const {offersCount, hotels, city, onUserCity, offersList} = props;
      ...
      <div className="tabs">
        <section className="locations container">
          <ul className="locations__list tabs__list">
            {citesNames.map((cityName, id) => <CityNameItem key={`cityName_${id}`} cityName={cityName} onUserCity={onUserCity} city={city} hotels={hotels} />)}
          </ul>
        </section>
      </div>
      <div className="cities">
        {getCitiesContainer(offersList, offersCount, city)}
      </div>
    </main>
  </div>;
};
